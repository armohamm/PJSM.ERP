namespace PUJASM.ERP.ViewModels.Master.Suppliers
{
    ...
    internal class MasterSuppliersVM : ViewModelBase
    {
        ...
        public ICommand ActivateSupplierCommand
        {
            get
            {
                return _activateSupplierCommand ?? (_activateSupplierCommand = new RelayCommand(() =>
                {
                    ...
                    if (_selectedLine.IsActive) DeactivateSupplierInDatabase(_selectedLine.Model);
        ...
        private void UpdateCategoryItems()
        {
            ...
            CategoryItems.Clear();
            var allCategoryItem = new Item {ID = "-1", Name = "All"};
        ...
        private void LoadDisplayedSuppliersAccordingToSelectedSupplier()
        {
            if (_selectedSupplier.Name.Equals("All"))
            {
                foreach (
                    var supplier in
                        Suppliers.ToList()
                            .Where(supplier => !supplier.Name.Equals("All") && supplier.IsActive.Equals(_isActiveChecked))
                    )
            ...
            {
                if (_selectedSupplier.IsActive.Equals(IsActiveChecked)) DisplayedSuppliers.Add(_selectedSupplier);
            }
        }
