namespace PUJASM.ERP.ViewModels.Reports
{
    ...
    public class InventoryReportVM : ViewModelBase
    {
        ...
        {
            ListedItems.Clear();
            var allItem = new Item { ID = "All", Name = "All" };
            ...
            using (var context = new ERPContext())
            {
                var itemsFromDatabase = context.Inventory.Include("ItemCategory").Include("Suppliers").Where(searchCondition).OrderBy(item => item.Name);
        ...
        private Func<Item, bool> GetSearchConditionAccordingToSelectedCategory()
        {
            if (_selectedCategory.Name.Equals("All"))
                return item => item.IsActive.Equals(_isItemActiveChecked);
            return item => item.ItemCategory.ID.Equals(_selectedCategory.ID) && item.IsActive.Equals(_isItemActiveChecked);
        }
        ...
        private Func<Item, bool> GetSearchConditionAccordingToSelectedSupplier()
        {
            if (_selectedSupplier.Name.Equals("All"))
                return item => item.IsActive.Equals(_isItemActiveChecked);
            return item => item.Suppliers.Contains(_selectedSupplier.Model) && item.IsActive.Equals(_isItemActiveChecked);
        }
